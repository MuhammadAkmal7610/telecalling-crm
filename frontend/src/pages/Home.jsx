...
const iconMap = {
    ...
    PhoneIcon, PlayCircleIcon, WrenchScrewdriverIcon,
    ShoppingCartIcon, ChatBubbleLeftRightIcon, BoltIcon,
    MegaphoneIcon, RectangleStackIcon, DevicePhoneMobileIcon
};
                ...
                <div className="mb-10 pt-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight">
                            Welcome back, <span className="text-teal-600">{user?.user_metadata?.full_name || user?.user_metadata?.name || 'User'}</span>!
                        </h1>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-4 gap-10">
                    ...
                    <div className="xl:col-span-3 space-y-12">
                        ...
                        <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            {[
                                { label: "Today's Leads", value: stats?.leads?.today || 0, icon: SignalIcon, color: 'text-blue-600', bg: 'bg-blue-50' },
                                { label: "Active Calls", value: stats?.calls?.active || 0, icon: PhoneIcon, color: 'text-green-600', bg: 'bg-green-50' },
                                { label: "Unread Msgs", value: stats?.messages?.unread || 0, icon: ChatBubbleLeftRightIcon, color: 'text-amber-600', bg: 'bg-amber-50' },
                                { label: "Offline Tasks", value: stats?.tasks?.pending || 0, icon: RectangleStackIcon, color: 'text-purple-600', bg: 'bg-purple-50' }
                            ].map((stat, i) => (
                                <div key={i} className="bg-white/60 backdrop-blur-xl border border-white/50 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-end gap-2">
                                                <p className="text-2xl font-black text-gray-900 leading-none">
                                                    {loadingStats ? <Skeleton className="h-6 w-12" /> : stat.value}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-gray-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-700"></div>
                                </div>
                            ))}
                        </section>
                        ...
                        <section className="relative z-10">
                            <div className="flex items-center justify-between mb-10 relative">
                                <div className="relative z-10">
                                    <h2 className="text-2xl font-black text-gray-900 tracking-tight">{config?.uiLabels?.getStartedTitle || 'Get Started'}</h2>
                                    <p className="text-base text-gray-500 mt-1 font-medium">{config?.uiLabels?.getStartedSub || 'Quick actions to setup your CRM.'}</p>
                                </div>
                                <button
                                    onClick={() => handleActionClick({ action: 'setup-guide' })}
                                    className="group text-teal-600 font-bold text-sm tracking-tight flex items-center gap-2 bg-white/80 backdrop-blur-md px-5 py-2.5 rounded-xl border border-teal-100 hover:bg-teal-600 hover:text-white transition-all duration-300 shadow-sm"
                                >
                                    <PlayCircleIcon className="w-5 h-5" />
                                    <span>{config?.uiLabels?.setupGuideBtn || 'Setup Guide'}</span>
                                </button>
                            </div>
                        ...
                        <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 p-6 overflow-hidden relative group hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500">
                            ...
                            <div className="flex gap-4 mb-5 relative z-10">
                                ...
                                <div className="flex-1 min-w-0 flex flex-col justify-center">
                                    <h3 className="font-black text-gray-900 text-lg leading-tight truncate tracking-tight">{config?.uiLabels?.appName || 'CRM App'}</h3>
